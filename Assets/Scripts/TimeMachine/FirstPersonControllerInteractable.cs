...
public class FirstPersonControllerInteractable : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        // Gets input and calls jump method
        if (enableJump && Input.GetKeyDown(interactKey))
        {
            Interact();
        }
    ...
    private void Interact()
    {
        ...
        Debug.DrawRay(ray.origin, ray.direction * maxInteractDistance, Color.red, 1f);

        if (Physics.Raycast(ray, out hit, maxInteractDistance, interactLayer))
        {
...
// Custom Editor
#if UNITY_EDITOR
[CustomEditor(typeof(FirstPersonControllerInteractable)), InitializeOnLoadAttribute]
public class FirstPersonControllerInteractableEditor : Editor
{
    FirstPersonControllerInteractable fpc;
    SerializedObject SerFPC;

    private void OnEnable()
    {
        fpc = (FirstPersonControllerInteractable)target;
    ...
    public override void OnInspectorGUI()
    {
        ...
        GUILayout.Label("Interact", new GUIStyle(GUI.skin.label) { alignment = TextAnchor.MiddleLeft, fontStyle = FontStyle.Bold, fontSize = 13 }, GUILayout.ExpandWidth(true));

        fpc.enableInteract = EditorGUILayout.ToggleLeft(new GUIContent("Enable Interact", "Determines if the player is allowed to interact."), fpc.enableInteract);
        GUI.enabled = fpc.enableInteract;
